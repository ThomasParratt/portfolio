...
export default function ProjectCard({p, detailTo}) {
    const visibilityClass = p.liveInternal ? "hidden lg:block" : "";
    return (
        <article className={`h-full border-4 border-white rounded-2xl p-4 shadow-xl transition bg-sky-700 ${visibilityClass}`}>
            {detailTo ? (
